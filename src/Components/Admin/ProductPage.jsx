...
function ProductPage({fetchAllProducts,products,setProducts}) {
    ...
    const submitform = async (event) => {
        ...
        try {
            // Submit initial product data without images to get the productId
            const productRes = await fetch(`${url}UploadProduct`, {
                method: 'POST',
                body: formData
            });
            ...
            const totalImages = images1.length + images2.length + images3.length + images4.length;
            let uploadedImagesCount = 0;
    
            // Define a function to chunk and send images
            const chunkAndSendImages = async (imagesArray, imageField) => {
                const chunkSize = 10;
                for (let i = 0; i < imagesArray.length; i += chunkSize) {
                    const chunk = imagesArray.slice(i, i + chunkSize);
                    const chunkFormData = new FormData();
                    chunkFormData.append('_id', productId);
                    chunk.forEach((image, index) => chunkFormData.append(imageField, image));
                    
                    const resChunk = await fetch(`${url}UploadImageChunk`, {
                        method: 'POST',
                        body: chunkFormData
                    });
                    console.log('resChunk', resChunk);
                    uploadedImagesCount += chunk.length;
                    const percentage = Math.round((uploadedImagesCount / totalImages) * 100);
                    setUploadProgress(percentage);
                }
            };
    
            // Chunk and send each type of images
            await chunkAndSendImages(images1, 'images1');
            await chunkAndSendImages(images2, 'images2');
            await chunkAndSendImages(images3, 'images3');
            await chunkAndSendImages(images4, 'images4');
        ...
        <div className='flex_wrap' style={{position:'relative'}} >
            ...
            <div style={{backgroundColor: "#eaeaea24"}} >
                ...
                <Box
                    ...
                    <div className="flex">
                        { sendLoading && <h5 className='m0' style={{margin:'0'}} > {uploadProgress}% </h5> }
